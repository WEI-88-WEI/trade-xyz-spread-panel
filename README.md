...
- 服务端仅在小时历史实际变化时才落盘，减少高频 WebSocket 推送带来的无意义写盘
- 图表层使用 ECharts，支持 dataZoom（鼠标滚轮缩放 + 底部 slider 拖拽/平移）
- 图表仅在当前小时 max / min 发生变化时刷新，减少实时推送对 dataZoom / tooltip 交互的干扰
...
当前提醒阈值不是写死值，而是两条动态阈值：

- **上穿阈值：最近 3 个小时桶的最大价差均值**
- **下穿阈值：最近 3 个小时桶的最小价差均值**

触发方式：

- 浏览器通知（需要授权）
- 提示音严格 6 次一组（使用单一 AudioContext 调度，单次触发后会播完，不因价格瞬时回落而中途取消；仍受浏览器前台/音频权限限制）
- 提醒冷却时间：10 秒
...
- 第一次上穿阈值时提醒一次
- 第一次下穿阈值时提醒一次
- 若价差一直保持在阈值外侧，不会连续重复提醒
- 回到阈值另一侧后重新复位，下次再次穿越再提醒
